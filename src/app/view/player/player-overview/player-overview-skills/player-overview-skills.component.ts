...
import { getLevelForXp, skillNames, skillOrder, Skills } from '../../../../shared/rs/rs.constants';
...
export class PlayerOverviewSkillsComponent implements OnInit {
    ...
    private refactorSkillsArray(skills: PlayerSkill[]): void {
        ...
        this._skillLevels = new Array(skillOrder.length);

        for(const skill of skills) {
            this._skills[ skill.id ] = skill;
            this._skillLevels[ skill.id ] = getLevelForXp(skill.xp, skill.id === Skills.INVENTION);
        }
    }
    ...
    public skillTooltip(skill: PlayerSkill): string {
        ...
        const tooltipArray = [ skillNames[ skill.id ],
            `Level: ${this._skillLevels[ skill.id ]}`,
